...
/**
 * Create grouped bar chart for display rates by decade
 * Shows display percentages for Male, Female, and Unknown side-by-side for each decade
 * @param {Array<string>} labels - Decade labels (e.g., ["1780s", "1790s", ...])
 * @param {Array<number>} maleData - Male display percentages
 * @param {Array<number>} femaleData - Female display percentages
 * @param {Array<number>} unknownData - Unknown display percentages
 * @param {string} canvasId - Canvas element ID
 * @param {Object} decadeData - Raw decade data with totals for sample sizes
 * @param {number} overallMaleRate - Overall male display rate for reference line
 * @param {number} overallFemaleRate - Overall female display rate for reference line
 * @returns {Chart} Chart.js instance
 */
export function createDisplayByDecadeChart(labels, maleData, femaleData, unknownData, canvasId, decadeData = {}, overallMaleRate = 0, overallFemaleRate = 0) {
  const ctx = getCanvasContext(canvasId);
  if (!ctx) return null;

  // Create reference line datasets for overall rates
  const overallMaleReference = Array(labels.length).fill(overallMaleRate);
  const overallFemaleReference = Array(labels.length).fill(overallFemaleRate);
  ...
  return new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        ...
        {
          label: 'Overall Male Rate',
          data: overallMaleReference,
          type: 'line',
          borderColor: CONFIG.colors.male,
          borderWidth: 2,
          borderDash: [5, 5],
        ...
        {
          label: 'Overall Female Rate',
          data: overallFemaleReference,
          type: 'line',
          borderColor: CONFIG.colors.female,
          borderWidth: 2,
          borderDash: [5, 5],
    ...
    options: {
      ...
      scales: {
        x: {
          title: { display: true, text: 'Acquisition Decade' }
        },
        y: {
          beginAtZero: true,
          max: 100,
          title: { display: true, text: 'Display Rate (%)' },
          ticks: {
            callback: function(value) {
              return value + '%';
            }
        ...
        tooltip: {
          callbacks: {
            label: function(context) {
              ...
              // For reference lines, show simple value
              if (datasetLabel.includes('Overall')) {
                return datasetLabel + ': ' + value + '%';
              }

              // For bars, add sample size
              const decadeIndex = context.dataIndex;
              const decadeKey = labels[decadeIndex].replace('s', ''); // "1780s" -> "1780"
              const decadeStats = decadeData[decadeKey];

              if (decadeStats) {
                const genderKey = datasetLabel; // "Male", "Female", or "Unknown"
                const total = decadeStats[genderKey]?.total || 0;
                const displayed = decadeStats[genderKey]?.displayed || 0;
                return `${datasetLabel}: ${value}% (${displayed}/${total} works)`;
              }
...
/**
 * Update display by decade chart
 * @param {Chart} chartInstance - Existing chart instance
 * @param {Array<string>} labels - Decade labels
 * @param {Array<number>} maleData - Male display percentages
 * @param {Array<number>} femaleData - Female display percentages
 * @param {Array<number>} unknownData - Unknown display percentages
 * @param {Object} decadeData - Raw decade data with totals for sample sizes
 ...
 * @param {number} overallFemaleRate - Overall female display rate for reference line
 */
export function updateDisplayByDecadeChart(chartInstance, labels, maleData, femaleData, unknownData, decadeData = {}, overallMaleRate = 0, overallFemaleRate = 0) {
  const overallMaleReference = Array(labels.length).fill(overallMaleRate);
  const overallFemaleReference = Array(labels.length).fill(overallFemaleRate);
  ...
  chartInstance.data.datasets[1].data = femaleData;
  chartInstance.data.datasets[2].data = unknownData;
  chartInstance.data.datasets[3].data = overallMaleReference;
  chartInstance.data.datasets[4].data = overallFemaleReference;

  // Update tooltip callback to use new decadeData
  chartInstance.options.plugins.tooltip.callbacks.label = function(context) {
    ...
    const decadeIndex = context.dataIndex;
    const decadeKey = labels[decadeIndex].replace('s', '');
    const decadeStats = decadeData[decadeKey];

    if (decadeStats) {
      const genderKey = datasetLabel;
      const total = decadeStats[genderKey]?.total || 0;
      const displayed = decadeStats[genderKey]?.displayed || 0;
      return `${datasetLabel}: ${value}% (${displayed}/${total} works)`;
    }
